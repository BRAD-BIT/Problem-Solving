...
vvll multi_matrix(vvll a, vvll b)
{
	...
	{
		lop(k, 0, b.size())c.mat[i][j] += (a[i][k] * b[k][j]);
	}
	return c.mat;
}
...
int main()
{
	...
	{
		if (!(cin >> n))break;;
		matrix ini(2, 2, 0); ini.mat[0][0] = 2, ini.mat[0][1] = p;
		matrix tran(2, 2, 1); tran.mat[0][0] = 0;  tran.mat[0][1] = -q; tran.mat[1][1] = p;
		if (n == 0)cout << 2 << endl;
		else
		if (n == 1)cout << p << endl;
		else
		cout << multi_matrix(ini.mat, matrix_power(tran.mat, n - 1))[0][1] << endl;
	}
}