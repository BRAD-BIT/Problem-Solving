...
void DFS(int node,int num,int change)
{
    vis[node] = 1; 
    for (int i = 0; i < graph[node].size(); i++)
    {
        int child = graph[node][i].first;
        if (!vis[child])DFS(child, num + graph[node][i].second - 1, graph[node][i].second == 2);
    }
    if (change)dis[node] = num;
}
int DFS2(int node)
{
    vis[node] = 1; int ans = 0;
    for (int i = 0; i < graph[node].size(); i++)
    {
        int child = graph[node][i].first;
        if (!vis[child])if (DFS2(child))dis[node] = -1, ans = 1;
    }
    return dis2[node] != -1 || ans;
}
int main()
{
